...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  account_locked: 'Cuenta bloqueada temporalmente. Contacta al administrador.',
...
export default function LoginForm({ initialError, redirectTo }: Props) {
  ...
  async function onSubmit(e: FormEvent) {
    ...
    // Login OK. Verificar que el rol sea superadmin.
    if (res.data?.user.role !== 'superadmin') {
      // Logout inmediato (no debería poder entrar acá)
      await api.post('/api/auth/logout');
      setError('Esta cuenta no tiene permisos de superadmin.');
      setLoading(false);
      return;
    }
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div>
        ...
        <input
          ...
          disabled={loading}
          className="w-full rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 px-3 py-2 text-slate-900 dark:text-white outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 disabled:opacity-50"
        />
      </div>
      <div>
        <label
